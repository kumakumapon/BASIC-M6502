...
class Bus:
    ...
    def __getitem__(self,a):
        ...
        if a==0x2002: self.latch=False; return 0x80
        if a==0x4017:
            bits=sum(1<<i for i in range(4) if self.keyboard_row*8+self.column*4+i in self.keys)
    ...
    def __setitem__(self,a,v):
        ...
        elif a==0x2000: self.control=v
        elif a==0x2006:
            self.ppu_address=((self.ppu_address&255)|(v<<8)) if not self.latch else ((self.ppu_address&0xff00)|v)
        ...
        elif a==0x2005: self.latch=not self.latch
        elif a==0x4016:
            col=(v>>1)&1
            if not col and self.column: self.keyboard_row=(self.keyboard_row+1)%10
            if v&1: self.keyboard_row=0
            self.column=col
            self.enabled=bool(v&4)
        elif 0x6000<=a<0x8000: self.ram[a]=v
        elif a in (0x8000,0xa000,0xe000): pass
        elif a in (0x2001,0x4010,0x4015,0x4017): pass
        else: self.bad_writes.append((a,v))
    ...
    def step(self):
        ...
        if c.processorCycles>=self.next_nmi:
            ...
            if self.bus.control&128 and self.nmi_start is None:
                ...
                c.nmi()
        if c.pc==self.labels['B_OUTCH']: self.output.append(chr(c.a&127))
    ...
    def run(self, text='', limit=8_000_000):
        ...
        for _ in range(limit):
            if self.cpu.pc==self.labels['B_INCHR']:
                if not self.inputs: return ''.join(self.output[start:])
                self.cpu.a=self.inputs.popleft()
                self.cpu.p=(self.cpu.p&~0x82)|(0x02 if self.cpu.a==0 else 0)|(self.cpu.a&128)
                self.cpu.pc=self.cpu.stPopWord()+1
            self.step()
        raise AssertionError(f'Timeout at {self.cpu.pc:04x}: {"".join(self.output[-200:])!r}')

    def call(self,label):
        self.cpu.stPushWord(0x5fff)
        self.cpu.pc=self.labels[label]
        for _ in range(100000):
            if self.cpu.pc==0x6000: return self.cpu.a
            self.step()
...
class RuntimeTests(unittest.TestCase):
    ...
    def command(self,text): return self.m.run(text+'\r')
    ...
    def test_arithmetic_and_strings(self):
        for cmd,want in [('PRINT 1+2',' 3 '),('PRINT "HELLO"','HELLO'),
                         ('PRINT 1.5*2.5',' 3.75 '),('PRINT 2^8',' 256 '),
                         ('PRINT SQR(81)',' 9 '),('PRINT LEN("ABCDE")',' 5 '),
                         ('PRINT MID$("ABCDE",2,3)','BCD'),
                         ('PRINT LEFT$("ABCDE",2)+RIGHT$("ABCDE",2)','ABDE')]:
            with self.subTest(cmd=cmd): self.assertIn(want,self.command(cmd))

    def test_program_editing_control_and_arrays(self):
        for line in ['10 DIM A(3):S=0','20 FOR I=1 TO 3:A(I)=I*2:S=S+A(I):NEXT I',
                     '30 GOSUB 100:PRINT S:END','100 S=S+1:RETURN']:
            self.command(line)
    ...
    def test_keyboard_shift_delete_repeat(self):
        ...
        for _ in range(24): result=poll([49])
