...
class Converter:
    def __init__(self):
        ...
        self.radix = 10
        self.out = ['; Generated from m6502.asm by scripts/convert.py', '.setcpu "6502"']
        self.overrides = dict(REALIO=5, BUFPAG=2, BUF=512, LINLEN=32,
                              BUFLEN=240, ROMLOC=32768, RAMLOC=24576,
                              NULCMD=0, GETCMD=1, DISKO=0, LONGI=0)
        self.serial = 0
    ...
    def expr(self, s):
        ...
        def default_number(m):
            n=0
            for digit in m[0]: n=n*self.radix+int(digit)
            return str(n)
    ...
    def statement(self, s, line):
        ...
        if label:
            ...
            if not s:
                return
        if re.match(r'^(TITLE|SEARCH|SALL|SUBTTL|PAGE|PRINTX|PURGE|END|XLIST|LIST|\.XCREF|\.CREF)\b', s, re.I):
            return
        ...
        if op == 'BLOCK':
            ...
            self.emit('.byte '+dict(ACRLF='13,10', SKIP1='$24', SKIP2='$2c')[op], line)
        elif op == 'SYNCHK':
            self.emit('lda #<('+self.expr(arg)+')\njsr B_SYNCHR', line)
        elif op in ('JEQ', 'JNE'):
            self.serial += 1
            self.emit(f'{"bne" if op=="JEQ" else "beq"} cv_{self.serial}\njmp {self.expr(arg)}\ncv_{self.serial}:', line)
        elif op == 'INCW':
            self.serial += 1
            a = self.expr(arg)
            self.emit(f'inc {a}\nbne cv_{self.serial}\ninc {a}+1\ncv_{self.serial}:', line)
            ...
            a = self.expr(arg)
            self.emit(('lda #0' if op == 'CLR' else f'lda {a}\neor #255')+f'\nsta {a}', line)
        elif op in ('LDWD','LDWDI','LDWX','LDWXI','LDXY','LDXYI','STWD','STWX','STXY','PULWD','PSHWD'):
            a = self.expr(arg)
            if op == 'PULWD':
                code = f'pla\nsta {a}\npla\nsta {a}+1'
            elif op == 'PSHWD':
                ...
                pair = ('a','y') if 'WD' in op else (('a','x') if 'WX' in op else ('x','y'))
            ...
            self.emit(op[:3].lower()+' ('+self.expr(arg)+('),y' if op[-1]=='Y' else ',x)'),line)
        elif op == 'JMPD':
            self.emit('jmp ('+self.expr(arg)+')',line)
        elif op in 'LDA LDX LDY STA STX STY ADC SBC CMP CPX CPY AND ORA EOR BIT INC DEC ASL LSR ROL ROR JMP JSR BCC BCS BEQ BNE BMI BPL BVC BVS PHA PHP PLA PLP TAX TAY TSX TXA TXS TYA INX INY DEX DEY RTS RTI CLC SEC CLI SEI CLD SED CLV NOP BRK'.split():
            a = self.expr(arg)
    ...
    def run(self, text):
        ...
        comment=False
        for line in lines:
            if line.startswith('COMMENT '): comment=line.split()[1]; clean.append(''); continue
            if comment:
                if line.strip() == comment: comment=False
        ...
        def block(pos, active=True, end=False):
            while pos<len(src):
                ...
                if src[pos]=='>':
                    if not end: raise ValueError(f'Unexpected > at {pos}')
                    return pos+1
                line=src.count('\n',0,pos)+1
                # A label may precede a conditional.
                lm=re.match(r'([\w$]+)::?\s*',src[pos:])
                if lm:
                    if active: self.statement(lm[0],line)
                    pos+=lm.end(); continue
                directive=re.match(r'(IFE|IFN|IF1|IF2|IFNDEF|REPEAT|DEFINE)\b',src[pos:],re.I)
                if directive:
                    ...
                    if op=='DEFINE':
                        pos=block(opening+1,False,True); continue
                    condition=False
                    count=1
                    if active:
                        if op in ('IF1','IF2','IFNDEF'): condition=False
                        elif op=='REPEAT': condition=True; count=self.value(head)
                        else:
                            if not head: raise ValueError(f'Empty {op} at line {line}')
                            condition=(self.value(head)==0)==(op=='IFE')
                    final=block(opening+1,active and condition,True)
                    for _ in range(count-1): block(opening+1,True,True)
                    pos=final; continue
                start=pos; depth=0; quoted=False
                while pos<len(src):
                    ch=src[pos]
                    if ch=='"': quoted=not quoted
                    if not quoted:
                    ...
                    pos+=1
                if active:
                    try: self.statement(src[start:pos],line)
                    except Exception as exc: raise ValueError(f'Line {line}: {src[start:pos]}') from exc
            if end: raise ValueError('Unclosed conditional')
            return pos
        block(0)
        result='\n'.join(self.out)+'\n'
        ...
        replace('B_LINLIN:', 'B_BUFOFS .set', '; INLIN / INCHR are supplied by nes/io.s')
        replace('lda #<(B_MEMORY)', 'B_ASKAGN:',
                '; Fixed cartridge RAM ceiling; never probe hardware registers.\n'
                'lda #0\nldy #$80\nsta B_MEMSIZ\nsty B_MEMSIZ+1\n'
                'sta B_FRETOP\nsty B_FRETOP+1')
        result=result.replace('.byte 83,84,77,32,66,65,83,73,67,32,86,49,46,49',
                              '.byte "NES BASIC V1.1"')
        return result
...
if __name__ == '__main__':
    ...
    (build/'basic.s').write_text(Converter().run((ROOT/'m6502.asm').read_text()),encoding='ascii')
